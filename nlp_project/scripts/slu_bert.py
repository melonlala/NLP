...
from model.slu_bert_decoder import SimpleDecoder, MultiTurnDecoder
...
from utils.initialization import set_random_seed
...
experiment_name = f'multibert.lr_{arguments.lr}.encoder_cell_{arguments.encoder_cell}.hidden_{arguments.hidden_size}.layer_{arguments.num_layer}.{datetime_now}'
...
for run, seed in enumerate(random_seeds):
    ...
    optimizer = Adam(decoder.parameters(), lr=arguments.lr, weight_decay=arguments.dropout)
    ...
    for epoch in range(arguments.max_epoch):
            ...
            for batch_x, batch_y in dev_data_loader:
                for round_x, round_y in zip(batch_x, batch_y):
                    for x, y in zip(round_x, round_y):
                        ...
                        # predictions.append(get_output(input_tokens, output, label_converter))
                        # labels.append(get_output(x.tokens_with_noise, y, label_converter))
            # metrics = evaluator.acc(predictions, labels)           
        acc = evaluator.bert_acc()
        f1_score = evaluator.f1_score()
        # acc = metrics['acc']
        # f1_score = metrics['fscore']
        avg_loss = total_loss.item() / len(dev_dataset)
        print(type(acc))
        print(type(f1_score))
        print(type(avg_loss))
        print(f'Acc: {acc:.5f}, F1 Score: {f1_score:.5f}, Avg. Loss: {avg_loss:.5f}')
        ...
        if acc > best_acc:
            ...
            torch.save({
                ...
                'seed': seed,
                'run': run,
            }, f'trained-models/slu-bert-multi-{arguments.encoder_cell}_{run}.bin')
