...
from model.bilstm_crf_tagging import BILSTMCRF
...
args.vocab_size = Example.word_vocab.vocab_size # 1741 vocab_size
args.pad_idx = Example.word_vocab[PAD] # pad_idm = 0
args.num_tags = Example.label_vocab.num_tags # num_tag = 74, label_num
args.tag_pad_idx = Example.label_vocab.convert_tag_to_idx(PAD) # tag_pad_id = 0, 补全的0label也为0


model = BILSTMCRF(args).to(device)
...
if not args.testing:
    ...
    for i in range(args.max_epoch):
        ...
        for j in range(0, nsamples, step_size):
            ...
            current_batch = from_example_list(args, cur_dataset, device, train=True)
            output, loss = model(current_batch) # (bs, length, num)
