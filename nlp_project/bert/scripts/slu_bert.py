...
if not args.testing:
    ...
    y2 = np.array(valid_loss)
    sns.set_style("whitegrid")
    plt.plot(x, y1, lw=1, c=color1, label='train_loss')  
    plt.plot(x, y2, lw=1, c=color2, label='valid_loss') 
    plt.xticks([0,10,20,30,40,50]) 

    plt.xlabel('epoch')  
    plt.ylabel('loss')  
    plt.legend()  
    plt.savefig('./bert-lstm') 
